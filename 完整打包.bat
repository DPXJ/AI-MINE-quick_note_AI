@echo off
chcp 65001 >nul
title QuickNote AI - 完整打包工具

echo ╔══════════════════════════════════════════╗
echo ║   QuickNote AI - 完整打包工具            ║
echo ╚══════════════════════════════════════════╝
echo.
...
echo [1/6] 检查是否已打包...
set HAS_PACKED=0
for /f "tokens=*" %%i in ('dir /b /ad "dist\QuickNote_AI_v*" 2^>nul') do (
    set HAS_PACKED=1
...
:check_done
if %HAS_PACKED%==1 (
    echo [检测] 发现已打包版本: %LATEST_VERSION%
    echo [提示] 将使用现有打包版本创建分发包
    echo.
) else (
    echo [检测] 未找到已打包版本
    echo [提示] 将先进行打包...
...
REM 提取版本号（去掉QuickNote_AI_v前缀）
for /f "tokens=2 delims=v" %%a in ("%LATEST_VERSION%") do set VERSION=%%a

echo [版本] 使用版本: %LATEST_VERSION% (V%VERSION%)
echo.
...
mkdir "%DIST_DIR%"
echo [成功] 分发目录已创建: %DIST_DIR%
echo.

REM 复制程序文件
echo [3/6] 复制程序文件...
xcopy /E /I /Y "dist\%LATEST_VERSION%\*" "%DIST_DIR%\%LATEST_VERSION%\" >nul
if errorlevel 1 (
    echo [错误] 复制程序文件失败
    pause
    exit /b 1
...
if exist "config.yaml" (
    ...
    echo [成功] config.yaml.example 已复制
) else (
    echo [警告] config.yaml 不存在，跳过
)

if exist "env.example" (
    copy /Y "env.example" "%DIST_DIR%\.env.example" >nul
    echo [成功] .env.example 已复制
) else (
    echo [警告] env.example 不存在，跳过
)
echo.
...
echo - `config.yaml.example` - 配置文件模板（可选，程序会自动创建）
echo - `.env.example` - 环境变量配置模板（可选，程序会自动创建）
echo.
echo ## 🚀 快速开始
echo.
echo ### 第一步：运行程序
...
echo 2. 双击 `QuickNote_AI_v%VERSION%.exe` 运行程序
...
REM 创建压缩包
echo [6/6] 创建压缩包...
set ZIP_NAME=QuickNote_AI_分发包_v%VERSION%.zip
