...
2. **配置 API 密钥**
   - 在 `QuickNote_AI_v3.1` 文件夹中，找到 `config.yaml` 和 `.env` 文件
   - 如果没有，请创建这两个文件（参考下面的配置说明）
...
在 `QuickNote_AI_v3.1` 文件夹中创建 `.env` 文件，内容如下：
...
在 `QuickNote_AI_v3.1` 文件夹中创建 `config.yaml` 文件，内容如下：
...
**解决方法**：
1. 检查 `logs` 文件夹中的日志文件
2. 确保 `.env` 和 `config.yaml` 文件存在且格式正确
3. 检查 API 密钥是否正确配置
